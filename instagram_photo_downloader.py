...
async def get_insta_photos(url: str, user_id: int, as_doc: bool = False):
    logger.info(f"--- ЗАПУСК БРАУЗЕРА (CLOAKBROWSER) ДЛЯ: {url} ---")
    downloaded_files = []
    media_urls = []
    try:
        ...
        logger.error(f"Ошибка при работе с CloakBrowser: {e}")
        try:
            if 'page' in locals():
                ...
                if resp.status == 200:
                    ...
                    if len(img_data) > 5000:
                        with open(final_filename, 'wb') as f:
                            f.write(img_data)
                        logger.info(f"Скачано фото {idx + 1} (Instagram)")
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_ig_photo(session, url, idx) for idx, url in enumerate(media_urls)]
